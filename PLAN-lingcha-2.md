...
> 最后更新：批 3 完成（磁盘 60 篇 / 60/300）

## 〇、当前指针（清空会话后只看这里）

| 项 | 值 |
|----|-----|
| **下一批** | **批 4** · 数据结构②：堆 / 前缀和 / 差分 / 单调队列 |
| 已完成 | 3/15 批（60/300） |
| 磁盘核对 | `ls solutions/lingcha-2/*.md \| wc -l` 应为 `60`；`60 ÷ 20 = 3` 已完成批 → 下一批 = 4 |
| 预查新题 | #3066 `minimum-operations-to-exceed-threshold-value-ii`；#3275 `k-th-nearest-obstacle-queries` |
| 样例 md | `solutions/lingcha-2/points-that-intersect-with-cars.md`（一维差分）或 `continuous-subarray-sum.md`（前缀和哈希） |
